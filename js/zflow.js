 ...
 */
 ...
 inappbrowser = window.open(newarray[i],'_blank', 'location=no');

 
 inappbrowser.addEventListener('loadstart', function(event) { thisstream="on";toogleStream(); console.log(event.type + ' - ' + event.url); } );
 inappbrowser.addEventListener('loadstop', function(event) { console.log(event.type + ' - ' + event.url); } );
 inappbrowser.addEventListener('exit', function(event) { console.log(event.type);});

 
 //transform += " translate3d(0, 0, 120px) rotateY(0deg)"; //mod by budgetappdev
 this.lastFocus = i;
 //$("#category_tickets").animate({bottom:"+=75px",leaveTransforms:true,useTranslate3d:true},400);
 }
 else
 {
 ...
 images.forEach(function (url, i)
                ...
                image.addEventListener("load", function ()
                                       ...
                                       webpage = image.src.replace("http://img.youtube.com/vi/","http:youtu.be/").replace("/0.jpg","");
                                  
                                       newarray.push(webpage);
