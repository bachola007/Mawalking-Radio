	...
	$.appManager = function(options){
		...
		*/
		...
		function injectTitlePage(title,login,i){
			if($("#appML_navigation_bar").length>0 && $("#appML_navigation_bar").is(".appML_auto_fill")){
				...
				if(login!=null)
					title="Login";
				$("#appML_navigation_bar").append($("<div style='position:relative'><div style='text-align:center;"+title_visibility+"'>MAWALKING RADIO</div><div class='back'>Back</div></div>"));
			}
		}
		...
		function injectMenuItems(title,icon,url,i){
			...
			if(isAppMLAutoFilled("appML_toolbar")){
				var html_toolbar="<div class='appML_toolbar_content' data-href='"+url+"' style='width:"+width+"%'>"+					
									"<div class='appML_toolbar_icon'><img style='margin:-4px 0px 0px -8px;' width='40' src='"+toolbar_icon_path+"/toolbar_icons/icons_off/"+icon+"' /></div>"+
			...
			if(isAppMLAutoFilled("appML_toolbar")){
				$("#appML_toolbar").find("[data-href]").each(function(index){
					...
					if(index!=n){
						...
						node.attr("src",src);
                        $(".appML_toolbar_label").eq(index).css( "color", "black" );
					}
					else{
						var	src_s = src.replace("icons_off","icons_on");
						node.attr("src",src_s);
                        $(".appML_toolbar_label").eq(index).css( "color", "red" );
		...
		*/
	    ...
	    function translateAppML(){
	    	...
	    	var loading_html=getAppMLDiv(prefix+"loading",null,null,"<div class='appMLLoading'><div style='text-align:center'><img src='../Default@2x.png' style='margin:0px -20px' width='320'></div></div>");
	    ...
	    //public functions
                       ...
                       this.refreshIscroll=function(id,newheight){
                       
                       var iScroll=getIScroll(id+"_appML_iscroll_pane");
                       iScroll.elem_dims.portrait.height = newheight;
                       console.log("resizing "+id+" to "+iScroll.elem_dims.portrait.height);
                       iScroll.refresh();
                       }; 
                       
                       this.iScrollTo = function(id,element){
                       var iScroll=getIScroll(id+"_appML_iscroll_pane");
                       console.log("scrolling "+id+" to "+iScroll);
                       iScroll.scrollTo(320*element,0,'350ms');
                       }
